...
def prepare_iemocap(
    ...
    # Check if this phase is already done (if so, skip it) - if these files exist skip it (assuming prepared)
    ...
    if different_speakers:
        # Constructs train, validation and test sets that do not share common speakers.
...
# Should add a way to map the transcription too?
...
# Why not keep excitement as well?
    ...
    for emoFile in [
        ...
        for utterance in load_utterInfo(pathEmo + emoFile):
            if (
                ...
                # I SEE it's because it's too similar to happy got it
                label = utterance[3]
                if label == "exc":
                    label = "hap"
